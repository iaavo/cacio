...
public class PTPRobotPeer implements RobotPeer {
     ...
     * 
     */
	private static final int BUTTON_MASKS = InputEvent.BUTTON1_MASK
	...
	public void mouseMove(int x, int y) {
		currentX = x;
		currentY = y;
		int id = isButtonPressed() ? MouseEvent.MOUSE_MOVED
	...
	public void mousePress(int buttons) {
		...
		if (buttonDownMask != 0 && buttonMask != 0) {
			...
			EventData ev = mouseEvent(MouseEvent.MOUSE_PRESSED, buttonDownMask,
					false);
	...
	public void mouseRelease(int buttons) {
		...
		if (buttonDownMask != 0 && buttonMask != 0) {
			EventData ev = mouseEvent(MouseEvent.MOUSE_RELEASED,
					buttonDownMask, (buttons & InputEvent.BUTTON3_MASK) != 0);
			CTCEventSource.getInstance().postEvent(ev);
	...
	private char getKeyCharFromCodeAndMods(int keyCode, int modifiers) {
		return KeyStrokeMappingFactory.getInstance().getKeyStrokeMapping()
				.getKeyChar(keyCode, modifiers);
	}
