...
import net.java.openjdk.awt.peer.web.CopyAreaScreenUpdate;
 ...
 */
public class PTPSurfaceData extends SurfaceData {

	static SurfaceType typeDefault = SurfaceType.IntRgb
			.deriveSubType("Cacio Web default");
	...
	protected PTPSurfaceData(PTPScreen screen, SurfaceType surfaceType,
		...
		data = ((DataBufferInt) imgBuffer.getRaster().getDataBuffer())
				.getData();

		imgBufferSD = SurfaceManager.getManager(imgBuffer)
				.getPrimarySurfaceData();

		int imgStride = ((SinglePixelPackedSampleModel) imgBuffer
				.getSampleModel()).getScanlineStride() * 4;
	...
	private native final void initOps(int[] data, int width, int height,
			int stride);
	...
	private boolean isOrderedScreenUpdateEfficient() {
		return BlitScreenUpdate
				.getPendingBlitScreenUpdateSize(surfaceUpdateList) * 2 < (bounds.width * bounds.height);
	}
	...
	public boolean copyArea(SunGraphics2D sg2d, int x, int y, int w, int h,
		...
		if (clipRect.isRectangular()
			...
			try {
				...
				surfaceUpdateList.add(new CopyAreaScreenUpdate(x, y, x + w, y
						+ h, dx, dy, clipRect));
			} finally {
				unlockSurface();
			}
