...
class CacioComponentPeer<AWTComponentType extends Component,
    ...
    public void setBounds(int x, int y, final int width, final int height, int op) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				if (proxy != null) {
					// Use the updated bounds from the awtCompnent here. The new
					// bounds
					// may be different from the given paramenters if 'op' was
					// SET_CLIENT_SIZE
					// or if this is a top level window and the system has
					// forced some
					// different bounds.
					proxy.setBounds(awtComponent.getX(), awtComponent.getY(),
							awtComponent.getWidth(), awtComponent.getHeight());
				...
				if (swingComponent != null) {
					// The swing component is laid out relative to the proxy
					// window.
					...
					// otherwise complex components (e.g. JScrollPane, etc) are
					// not
					// laid out correctly.
