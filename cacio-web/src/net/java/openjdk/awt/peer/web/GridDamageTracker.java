...
public class GridDamageTracker {
  ...
  public GridDamageTracker(int width, int height) {
    this.rectList = new ArrayList<WebRect>();

    final int cellsX = (int)Math.ceil(((double)width) / GRID_SIZE);
    final int cellsY = (int)Math.ceil(((double)height) / GRID_SIZE);
    this.grid = new DamageGridCell[cellsY][cellsX];

    for (int y = 0; y < cellsY; y++) {
      for (int x = 0; x < cellsX; x++) {
        this.grid[y][x] = new DamageGridCell(x * GRID_SIZE, y * GRID_SIZE);
      }
    }
  }
  ...
  public void trackDamageRect(WebRect rect) {
    final int x1Cell = rect.getX1() / GRID_SIZE;
    final int y1Cell = rect.getY1() / GRID_SIZE;
    final int x2Cell = Math.min(rect.getX2() / GRID_SIZE, this.grid[0].length - 1);
    final int y2Cell = Math.min(rect.getY2() / GRID_SIZE, this.grid.length - 1);

    // Adds the rect to each cell underneath the rect's area
    for (int y = y1Cell; y <= y2Cell; y++) {
      for (int x = x1Cell; x <= x2Cell; x++) {
        this.grid[y][x].addDamageRect(rect);
      }
    }
  ...
  public List<ScreenUpdate> groupDamagedAreas(BufferedImage imgBuffer) {
    final WebRect unionRect = getUnionRectangle();
    if (unionRect != null) {
      final ArrayList<ScreenUpdate> screenUpdateList = new ArrayList<ScreenUpdate>();

      final List<WebRect> regionList = createDamagedRegionList(5);
      if (unionRect != null && unionRect.getWidth() > 0 && unionRect.getHeight() > 0) {
        for (final WebRect dRect : regionList) {
          screenUpdateList.add(new BlitScreenUpdate(dRect.getX1(), dRect.getY1(), dRect.getX1(), dRect.getY1(), dRect
            .getWidth(), dRect.getHeight(), imgBuffer));
        }
      }
  ...
  private WebRect getUnionRectangle() {
    ...
    return new WebRect().union(this.rectList);
  }
  ...
  private List<WebRect> createDamagedRegionList(int mergeLimit) {
    final WebRect[][] unions = new WebRect[this.grid.length][this.grid[0].length];
    final List<WebRect> rectList = new ArrayList<WebRect>();

    // Calculate damaged region for each Cell
    for (int y = 0; y < this.grid.length; y++) {
      for (int x = 0; x < this.grid[0].length; x++) {
        final DamageGridCell elem = this.grid[y][x];
  ...
  /*
   * Internal debug function for counting the number of independent unions.
  ...
  private void mergeCellsHorizontal(WebRect[][] unions, int mergeLimit) {
    for (int y = 0; y < unions.length; y++) {
      for (int x = 0; x < unions[0].length - 1; x++) {
        final WebRect cellRect = unions[y][x];

        final int firstedMerged = x;
        for (x++; x < unions[0].length && cellRect != null && (x - firstedMerged) < mergeLimit; x++) {
          final WebRect extensionRect = unions[y][x]; // Next rectangle
          // horizontally

          // If the next rectangle has the same dimensions, discard it
          // and expand the current one to cover its area.
          if (extensionRect != null && extensionRect.y1 == cellRect.y1
              && extensionRect.getHeight() == cellRect.getHeight() && extensionRect.x1 == cellRect.x2) {
    ...
    for (int x = 0; x < unions[0].length; x++) {
      for (int y = 0; y < unions.length - 1; y++) {
        ...
        for (y++; y < unions.length && cellRect != null && (y - firstMergedCell) < mergeLimit; y++) {
          ...
          // If the next rectangle has the same dimensions, discard it
          // and expand the current one to cover its area.
          if (extensionRect != null && extensionRect.x1 == cellRect.x1
              && extensionRect.getWidth() == cellRect.getWidth() && extensionRect.y1 == cellRect.y2) {
...
class DamageGridCell {
  ...
  protected void addDamageRect(WebRect rect) {
    if (this.cellRect == null) {
      this.cellRect = new WebRect(rect);
    } else {
      this.cellRect.union(rect);
    }
  }
  ...
  protected WebRect calculateDamageUnion() {
    ...
    final int x2 = this.x + GridDamageTracker.GRID_SIZE;
    final int y2 = this.y + GridDamageTracker.GRID_SIZE;
    this.cellRect.restrictToArea(this.x, this.y, x2, y2);

    if (this.cellRect.getWidth() == 0 || this.cellRect.getHeight() == 0) {
      return null;
    }
