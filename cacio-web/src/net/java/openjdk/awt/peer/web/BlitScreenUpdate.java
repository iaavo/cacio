...
public class BlitScreenUpdate extends ScreenUpdate {
  ...
  public void evacuate() {
    if (!this.isEvacuated) {
      final BufferedImage src = this.image;

      final int w = getUpdateArea().getWidth();
      final int h = getUpdateArea().getHeight();

      this.image = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
      final Graphics g = this.image.getGraphics();
      g.drawImage(src, 0, 0, w, h, this.srcX, this.srcY, this.srcX + w, this.srcY + h, null);
      this.srcX = 0;
      this.srcY = 0;
  ...
  public void writeToCmdStream(List<Integer> cmdList) {
    ...
    cmdList.add(this.packedX);
    cmdList.add(this.packedY);
  }
  ...
  public static int getPendingBlitScreenUpdateSize(List<ScreenUpdate> pendingUpdates) {
    ...
    for (final ScreenUpdate update : pendingUpdates) {
      if (update instanceof BlitScreenUpdate) {
        final WebRect updateArea = update.getUpdateArea();
  ...
  public WebRect getSurfaceDataUpdateArea() {
    return new WebRect(this.originalSrcX,
                       this.originalSrcY,
                       this.originalSrcX + this.updateArea.getWidth(),
                       this.originalSrcY + this.updateArea.getHeight());
  }
